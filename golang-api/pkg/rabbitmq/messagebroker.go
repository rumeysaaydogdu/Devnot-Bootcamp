...
func (b *broker) CreateConnection(parameters MessageBrokerParameter) error {
	...
	for {
		...
		if b.connection, err = amqp.Dial(b.chooseNode()); err != nil {
			fmt.Printf("application retries to connect rabbitmq : %s \n", err.Error())
			time.Sleep(reconnectDelay)
			continue
		}
		b.onClose()
		fmt.Println("application connected rabbitMq")
...
func (c *Channel) Publish(ctx context.Context, routingKey string, exchangeName string, payload interface{}) error {
	var message, err = publishExchangeToMessage("", payload)

	if err != nil {
		return err
	}
	return c.rabbitChannel.Publish(exchangeName, routingKey, false, false, message)
}
...
func (c *Channel) listenToQueue(queueName string) (<-chan amqp.Delivery, error) {

	msg, err := c.rabbitChannel.Consume(queueName,
		"",
		false,
		false,
		false,
